...
export default function ColumnTable({ columns, plan, selectedColumn, onSelectColumn }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white/65 shadow-sm backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-white/70 px-5 py-4">
        <div>
          <p className="text-sm font-heading font-extrabold text-foreground">Column profile table</p>
          <p className="text-xs text-muted-foreground">{filtered.length} of {columns.length} columns</p>
        </div>
        ...
        <table className="w-full min-w-[900px] border-collapse text-sm">
          ...
          <tbody>
            {filtered.map((column) => {
              ...
              const originalType = column.inferred_type || 'text';
              const effectiveType = item ? getEffectiveTypeFromColumnPlan(item) : originalType;
                ...
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-8 w-8 items-center justify-center rounded-xl" style={{ background: `${meta.color}14`, color: meta.color }}>
                        <Icon className="h-4 w-4" />
                      </div>
                      <div>
                        <p className="font-heading font-bold text-foreground">{column.column_name}</p>
                        <p className="text-[10px] text-muted-foreground">{column.raw_dtype || 'unknown dtype'}</p>
                        {effectiveType !== originalType ? (
                          <p className="text-[10px] text-muted-foreground">Cast from <span style={{ color: originalMeta.color }}>{originalMeta.label}</span></p>
                        ) : null}
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex flex-col items-start gap-1">
                      ...
                      {effectiveType !== originalType ? (
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-muted-foreground">casted</span>
                      ) : null}
                    </div>
                  </td>
